...
class verify():
    # 网格管理员进入待核实列表
    def test_app_wggly_heShiList(self,loginUser):
        ...
        if 'count' in dqr_res:
            ...
            print("待核实列表查询失败")
    # 网格管理员核实
    def test_app_wggly_daiHeShiDetail(self,loginUser):
        dhsResult = self.test_app_wggly_heShiList(loginUser)
        if dhsResult != None:
            ...
            if count>'0':
                ...
                hs_data = {
                    ...
                    "confirmid.id":loginUser['id'],#核实人id
                    "mposl":dhs_mposl,
                ...
                if '<issuc>true</issuc>' in hs_result:
                    print("核实：网格管理员",loginUser['name'],"核实完毕")
                    return True
                else:
                    print("核实：网格管理员",loginUser['name'],"核实失败")
                    return False
                
    #****************************************************************************************************************

    # 执法局待核实列表
    # def test_app_zfj_heShiList(self):
    ...
    #         print("待核实列表查询失败")
    # #执法局核实
    # def test_app_zfj_daiHeShiDetail(self):
    #     dhs_Result = self.test_app_zfj_heShiList()
    #     if dhs_Result != None:
