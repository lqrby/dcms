...
class RequestThread(threading.Thread):  
    # 构造函数  
    def __init__(self, thread_name,loginUrl,personData):  
    ...
    def userLogin(self):
        # print("url:",loginUrl)
        loginData = {
            "role":self.personData['jobposition'],
            "logonname":self.personData['logonname'],
            "logonpassword":self.personData['password']
        }  
        response = requests.post(loginUrl,loginData,timeout=1,allow_redirects=False)
        return response
        # loginResult = json.loads(response.text)
        # if 'message' in loginResult and loginResult['message'] == 'success':
        #     print("登录成功",self.personData['logonname'])
        #     return response
        # else:
        #     print("登录失败！！！",loginResult,personData['line'])
