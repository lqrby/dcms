...
class reviewAndReturnVisit():
    ...
    #进入待回访案卷详情并回访
    def test_returnDetailsAndVisit(self,hfItem):
        # 获取待回访列表
        ...
        if int(number)>0:
            ...
            hf_data = {
                ...
                "resultprocess":hfItem['resultprocess'],
                "isFaction":"",
                "myd":"on",
                "operatingComments":hfItem['operatingComments'],
                "sentence":""
            }
            hf_header = {
                "Cookie":dclxq_cookies
...
#执法局查询案卷列表    
    ...
    # ==================================================================================================
    #网格员apk查询案卷列表    
    def test_app_returnVisitList(self,fh_item):
        dhf_url = getConstant.IP_WEB_180+"/dcms/pwasCase/Case-checkList.action"
        wgglydfh_data = {
            "page.pageSize":"20",
            "bgadminid.id":fh_item['id'],
            "page.pageNo":"1"
        }
    ...
    def test_app_returnDetailsAndVisit(self,fh_item):
        # 获取列表
        dfh_list = self.test_app_returnVisitList(fh_item)
        if dfh_list != False:
            dcl_count = re.compile('<caseCheckList count="(.*?)"').search(dfh_list).group(1)
            if int(dcl_count)>0:
                wgglydclId = re.compile('<id>(.*?)</id>').search(dfh_list).group(1)
                cl_url = getConstant.IP_WEB_180+"/dcms/pwasCase/Case-pdacheckCase.action"
                cl_data = {
                    "id":wgglydclId,
                    "checkdesc":fh_item['checkdesc'],
