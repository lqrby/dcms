...
class XQD():
    ...
    # def XQDOneList(self):
    #     oneClassIdArr = []
    #     xqd_one_url = self.ip+'/partselection/poilyrs/getByParentid.do?parentid=-1'
    #     xqd_one_res = requests.get(xqd_one_url,headers = self.header,allow_redirects = False,timeout = 20)
    #     xqd_one_res.connection.close()
    #     if "服务调用成功" in xqd_one_res.text:
    #         oneListResult = json.loads(xqd_one_res.text)
    #         oneList_Result = oneListResult['result']
    #         for oneObj in oneList_Result:
    #             oneClassIdArr.append(oneObj['classid'])
    #         return oneClassIdArr
    #     else:
    #         print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX查询所有兴趣点一级大类失败XXXXXXXXXXXXXXXXXXXXXXXX")
    #         return False

    #根据一级分类获取二级分类列表
    # def XQDTwoList(self,classid):
    #     twoArr = []
    #     if classid != False:
    #         xqd_two_url = self.ip+'/partselection/poilyrs/getByParentid.do?parentid='+classid+'&_='+str(round(time.time()*1000))
    #         twoListRes = requests.get(xqd_two_url,headers = self.header,allow_redirects = False,timeout = 20)
    #         twoListRes.connection.close()
    #         if '服务调用成功' in twoListRes.text:
    #             twoListResult = json.loads(twoListRes.text)
    #             twoList_Result = twoListResult['result']
    #             for twoResult in twoList_Result:
    #                 twoArr.append(twoResult['classid'])
    #             return twoArr
    #         else:
    #             print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX查询所有兴趣点二级大类失败XXXXXXXXXXXXXXXXXXXXXXXX")
    #             return False
    ...
    #根据一、二级分类获取三级分类
    # def XQDThreeList(self,oneClassid,twoClassid):
    #     # xqd_three_url = self.ip+'/partselection/poilyrs/getListSanjiByPage.do?curPage=1&pageSize=10&classid1='+one_classid+'&classid2='+two_classid+'&classid='+three_classid+'&_='+str(round(time.time()*1000))
    #     xqd_three_url = self.ip+'/partselection/poilyrs/getListSanjiByPage.do?curPage=1&pageSize=10&classid1='+oneClassid+'&classid2='+twoClassid+'&_='+str(round(time.time()*1000))
    #     threeListRes = requests.get(xqd_three_url,headers = self.header,allow_redirects = False,timeout = 20)
    #     threeListRes.connection.close()
    #     if '服务调用成功' in threeListRes.text:
    #         threeListResult = json.loads(threeListRes.text)
    #         return threeListResult['result']['list']
    #     else:
    #         print("XXXXXXXXXXXXXXXXXXXXX获取三级兴趣点图标列表失败XXXXXXXXXXXXXXXXXXXX")
    #         return False
    ...
    def XQDDetailsAndUpdeit(self,threeItem):
        ...
        if os.path.isfile(listIcon) and os.path.isfile(pointIcon):
            ...
            if '服务调用成功' in res.text:
                ...
                print("兴趣点图标{}上传失败".format(threeItem["classid3"]))
                # return False
        else:
            print(listIcon+"或"+pointIcon+"路径下的文件不存在")
            # return False
