...
class setUpCase():
    ...
    def test_toBePutOnRecordList(self):
        ...
        if '<span id="pagemsg"' in respons.text:
            listcount = re.compile('<label>总共(.*?)页,(.*?)条记录</label>').search(respons.text).group(2)
            if listcount > '0':
                dailianresult = BeautifulSoup(respons.text,'html.parser')
                liandivObj = dailianresult.find('div', attrs={'class': 'mainContentTableContainer'})
                liandivObj.findAll('table')[0].extract()
                table = liandivObj.findAll('table')
                table[0].findAll('tr')[0].extract()
    ...
    def test_detailsAndFiling(self):
        # 获取待立案列表
        orderId = self.test_toBePutOnRecordList()
        if orderId:
